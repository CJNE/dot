...
puts "Using ami #{ami.name} #{family} #{instance_type} (#{virt})"
...
packer_cmd = "packer build -var 'ami=#{ami.name}' -var 'instance_type=#{instance_type}' -var 'ami-name=#{ARGV[0]}' -var 'family=#{family}' -var \"run_list=#{ARGV[1]}\" packer/builders/ec2-ebs.json"
